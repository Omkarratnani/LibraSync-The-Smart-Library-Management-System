...
student_credentials = {'username': 'student', 'password': 'pass1'}
...
def open_library_page(username):
    library_window = Tk()
    library_window.title(f'Library Page - {username}')
    # ... (add widgets and functionality for the library page)
...
# Login button
btnLogin = Button(login_window, text='Login', command=login)
...
def BorrowBook():
    ...
    global BorrowIDEntry, BorrowNameEntry
    ...
    try:
        ...
        bookBorrowLabel = Label(bookBorrowFrame, text='Borrow Book', bg='black', fg='white',
        ...
        BorrowNameLabel = Label(bookBorrowInfoFrame, text='Borrower Name : ', bg='black', fg='white', font=fonts)
        BorrowNameLabel.place(relx=0.07, rely=0.59)

        BorrowNameEntry = Entry(bookBorrowInfoFrame)
        BorrowNameEntry.place(relx=0.28, rely=0.62, relwidth=0.62)
...
def Borrow():
    ...
    borrowName = BorrowNameEntry.get().strip()
    ...
    if len(borrowID) == 3:
        try:
            ...
            if booksInfoBorrow[borrowID][-1] == 'not taken':
                ...
                file.close()
                # Add the borrower information to the data
                with open('src/Borrowers.txt', 'a') as borrowers_file:
                    borrowers_file.write(f'{borrowID} - {booksInfoBorrow[borrowID][0]} - {booksInfoBorrow[borrowID][1]} - {borrowName}\n')
                messagebox.showinfo(
                    title='Successfully borrowed',
                    message=f'The book : {booksInfoBorrow[borrowID][0]}\n'
                            f'written by : {booksInfoBorrow[borrowID][1]}\n'
                            f'with ID : {borrowID}\n'
                            f'successfully borrowed by {borrowName}.\n')
            else:
                messagebox.showerror(title='Already taken', message='This book has already been taken.\n'
                                                                     'Please choose another book.')
        except FileNotFoundError:
            pass

    else:
        messagebox.showerror(title='Wrong ID', message='Please enter a valid 3-digit ID.\n')
